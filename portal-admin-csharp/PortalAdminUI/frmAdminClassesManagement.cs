...
{
    public partial class frmAdminClassesManagement : Form
    {
        public frmAdminClassesManagement()
        {
            InitializeComponent();
        }

        private async void AdminClassesManagement_Load(object sender, EventArgs e)
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        List<AdminClassesDTO> AdminClassesDTOs = JsonConvert.DeserializeObject<List<AdminClassesDTO>>(jsonResponse);
                        dgvAdminclass.DataSource = AdminClassesDTOs;
