...
{
    public partial class frmCurriculumManagement : Form
    {
        public frmCurriculumManagement()
        {
            InitializeComponent();
        }

        private async void CurriculumManagement_Load(object sender, EventArgs e)
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    // 2. GỌI API
                    string apiUrl = $"http://localhost:8080/UniversityPortalBackend/api/admin/curriculums";
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        List<CurriculumDTO> curriculumDTOs = JsonConvert.DeserializeObject<List<CurriculumDTO>>(jsonResponse);
                        dgvCurriculum.DataSource = curriculumDTOs;
