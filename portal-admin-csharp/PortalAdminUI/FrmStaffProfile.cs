...
namespace PortalAdminUI
{
    public partial class FrmStaffProfile : Form
    {
        ...
        private async void FrmStaffProfile_Load(object sender, EventArgs e)
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        List<StaffProfileDTO> staffProfileDTOs = JsonConvert.DeserializeObject<List<StaffProfileDTO>>(jsonResponse);
                        dgvStaff.DataSource = staffProfileDTOs;
                    }
                    else
                    {
